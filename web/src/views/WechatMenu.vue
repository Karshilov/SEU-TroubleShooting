<template>
  <div id="type-view" class="page">
    <div>
      ...
      <div class="title-hint">添加部门员工，只有添加到该部门的员工才能处理部门分管的故障</div>
      <div class="content">
        <el-form label-width="80px">
          <el-form-item label="一卡通号">
            <el-input v-model="staffCardnum" placeholder="员工的一卡通号"></el-input>
          </el-form-item>
          <el-form-item>
  ...
  methods: {
    async add() {
      let res = await this.$axios.post(
        "/department/staff",
        { departmentId: this.departmentId, staffCardnum:this.staffCardnum },
    ...
    async load() {
      let res = await this.$axios.get("/department/staff?departmentId="+this.departmentId, {
        headers: { token: this.token }
      });
    ...
    async deleteStaff(staffCardnum) {
      let res = await this.$axios.delete("/department/staff?staffCardnum="+staffCardnum+"&departmentId="+this.departmentId, {
        headers: { token: this.token }
      });
      this.load()
    }
